...
const RidesDriver = () => {
  ...
  const getUserOrders = async () => {
    try {
      ...
      const { data } = await axiosInstance.get(`/all-orders-driver/${id}`);
      console.log("getUserOrders", data);
      if (data?.success) {
        const mybookings = data?.bookings;
        setOrder(mybookings.reverse());
        setOrderPlace(data?.bookings.length);
      }
      ...
      <div className={`card p-3 ${order.status === "0" && "bg-danger-light"}`}>
        ...
        {order.userId.length !== 0 ? (
          <div className="d-flex">
            ...
            {order.status !== "0" && order.endStatusOTP !== 1 && (
              <div className="d-flex gap-2 ms-auto">
                ...
                <Link
                  to={`/driver/chat/${order.userId?._id}/${order._id}/`}
                  className="dz-icon icon-sm icon-fill"
                >
          ...
          <div className="alert alert-warning solid alert-dismissible fade show">
            ...
            <strong className="fs-6"> Waiting For Driver To Accept ....</strong>
            <button
              type="button"
              className="btn-close"
        ...
        <div className="d-flex justify-content-between">
          {order.status === "0" ? (
            <button
              type="button"
              className="btn btn-sm mb-2 me-2 btn-primary ms-auto"
              disabled
            >
              View Ride <i className="ri-arrow-right-line ms-2"></i>
            </button>
          ) : (
            <Link
              to={`/driver/rides/${order.userId?._id}/${order._id}/`}
              className="btn btn-sm mb-2 me-2 btn-primary ms-auto"
            >
              View Ride <i className="ri-arrow-right-line ms-2"></i>
            </Link>
