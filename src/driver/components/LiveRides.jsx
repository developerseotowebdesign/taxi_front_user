  ...
  const decryptdatajson = getDecryptData();
   ...
   const DriverId = decryptdatajson._id;
        ...
        const fetchLiveRides = async () => {
            if (AllRideReq && AllRideReq.length !== 0) {
              ...
              sound.play();
                ...
                setLoading(true);
                try {
                    const response = await axiosInstance.get(`/get-all-ride/${DriverId}`);
    ...
    const AcceptRide = async (orderId) => {
      ...
      try {
            const response = await axiosInstance.post('/accept-order',Ridedata);
...
{Loading ? (<>
    ...
    {Mybooking.slice(-1).map((booking, index) => (
          <div className="card p-3" key={index}>
            <div className="d-flex">
              ...
              <div className="media-body">
                <h5 className="mt-2 mb-0">{booking?.userId[0]?.username}</h5>
                <div className="d-flex flex-row justify-content-between align-text-center">
                  <small className="text-muted">{booking.PickupLocation}</small>
                </div>
              </div>
            </div>
            ...
            <div className="d-flex justify-content-between">
              ...
              <button type="button" onClick={() => AcceptRide(booking._id)}className="btn btn-sm mb-2 me-2 btn-primary">
        ...
        <Link className="btn btn-primary btn-sm m-auto" to="/driver/home" onClick={CloseFoundRide} // Change onclick to onClick
 > <i class="ri-route-fill me-1"></i> All Rides</Link>
        </div>
