...
const Chat = () => {
  ...
  useEffect(() => {
    ...
    const fetchUserById = async () => {      
      ...
      try {
        const response = await axiosInstance.get(`/get-message-orderid/${userId}/${senderId}/${bookid}`);
  ...
  const handleMessageSubmit = async (e) => {
    ...
    e.preventDefault();
    const sendmsg = { text: messageInput, userId:userId,senderId:senderId ,orderId:bookid,type:"message"}
    sendMessage(sendmsg);
    ...
    try {
         ...
         const {data} = await axiosInstance.post(`/add-message-orderid`,  { text: messageInput, userId:userId,senderId:senderId,orderId:bookid });
         const { success } = data;
  ...
  const scrollToBottom = () => {
    ...
    window.scrollTo({ top: document.body.scrollHeight + scrollOffset, behavior: 'smooth' }); // Scroll to the bottom of the page with an additional offset
  };
...
const formatTimestamp = (timestamp) => {
  ...
  return (
    <>
      <header className="header header-fixed rounded-bottom ">
        <div className="header-content ">
          ...
          <div className="mid-content">
            <h4 className="title">{Loading ? (
                  <div className='skeleton m-auto'  style={{ height: 35, width: 150 }} />
            ):(<>
      ...
      <main className="page-content dz-gradient-shape  space-top p-b60 ">
        <div className="container">
          ...
          {Loading ? (<>
            <div  className="chat-content" >
              ...
              <div key={index} className={`chat-content  ${message.receiver
                === userId ? 'user' : ''}`}>
  ...
  message.orderId === bookid && (
    <div key={index} className={`chat-content ${message.userId === userId ? 'user' : ''}`}>
      <div className="message-item">
        <div className="bubble">{message.text}</div>
